...
function simulateDataUpdate() {
    // Temperatura Centro Historico
    firebase.database().ref('/smartcity/sensors/Temperatura_CentroHistorico').set({
        value: (20 + Math.random() * 5).toFixed(1),
        dataTime: new Date().toISOString(),
    });

    // Temperatura Parque Verde
    firebase.database().ref('/smartcity/sensors/Temperatura_ParqueVerde').set({
        value: (20 + Math.random() * 5).toFixed(1),
        dataTime: new Date().toISOString(),
    });

    // Temperatura Zona Industrial
    firebase.database().ref('/smartcity/sensors/Temperatura_ZonaIndustrial').set({
        value: (20 + Math.random() * 5).toFixed(1),
        dataTime: new Date().toISOString(),
    });

    // Temperatura Zona Residencial
    firebase.database().ref('/smartcity/sensors/Temperatura_ZonaResidencial').set({
        value: (20 + Math.random() * 5).toFixed(1),
        dataTime: new Date().toISOString(),
    });

    // Humidade Centro Historico
    firebase.database().ref('/smartcity/sensors/Humidade_CentroHistorico').set({
        value: (60 + Math.random() * 15).toFixed(1),
        dataTime: new Date().toISOString(),
    });

    // Humidade Zona Residencial
    firebase.database().ref('/smartcity/sensors/Humidade_ZonaResidencial').set({
        value: (60 + Math.random() * 15).toFixed(1),
        dataTime: new Date().toISOString(),
    });

    // Luminosidade Centro Historico
    firebase.database().ref('/smartcity/sensors/Luminosidade_CentroHistorico').set({
        value: (300 + Math.random() * 200).toFixed(1),
        dataTime: new Date().toISOString(),
    });

    // Luminosidade Parque Verde
    firebase.database().ref('/smartcity/sensors/Luminosidade_ParqueVerde').set({
        value: (300 + Math.random() * 200).toFixed(1),
        dataTime: new Date().toISOString(),
    });
    
    // Luminosidade Zona Residencial
    firebase.database().ref('/smartcity/sensors/Luminosidade_ZonaResidencial').set({
        value: (300 + Math.random() * 200).toFixed(1),
        dataTime: new Date().toISOString(),
    });
